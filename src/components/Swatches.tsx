...
export function HeroGate() {
  return (
    <div className="rounded-2xl border border-zinc-700 bg-zinc-900/60 p-4">
      <AccentGateViz
        title="Example: black gate frame with lime accent hardware (illustration)"
        className="h-64 w-full sm:h-80"
      />
      <p className="mt-2 text-center text-xs text-zinc-400">
        Illustration: stock-looking black frame, lime handles / hinges / drop
        rod. {SWATCH_DISCLAIMER}
      </p>
    </div>
  );
}
